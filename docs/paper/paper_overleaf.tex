...
\end{itemize}
\begin{equation}
    R_{\text{\1}} = r_P - \lambda_{\text{\1}}\,\text{CVaR}_{\beta}[L_P] - \lambda_{\text{\1}}\,\text{DD}_t - \sum_j \mu_j\,\mathbb{1}[\text{constraint}_j\ \text{violated}]
...
\item   \textbf{Action Space ($A_{\text{\1}}$):} Execution decisions including order sizing $q_t$, timing parameters, execution strategy selection (TWAP/VWAP/IS-minimizing), and order splitting for large positions.

\item   \textbf{Reward Function:} Implementation shortfall minimization with execution quality metrics:
\end{itemize}
\begin{equation}
    R_{\text{\1}} = -\operatorname{IS}_t - \lambda_{\text{\1}}c_{\text{\1}} - \lambda_{\text{\1}}\text{Impact}(|q_t|) + \lambda_{\text{\1}}\text{FillRate}_t
    
    where $\operatorname{IS}_t$ is implementation shortfall, $c_{\text{\1}}$ are transaction costs, and $\text{FillRate}_t$ rewards execution speed.

\textbf{Hierarchical Coordination Mechanism:}
The SPA generates strategic directives $\mathbf{d}_t = [\mathbf{w}^{target}, \rho_{\text{\1}}, \text{urgency}]$ that serve as binding constraints for EOA. True hierarchical enforcement is achieved through:

\end{equation}

\1ath
\begin{equation}
\pi_{\text{\1}}(a\_{\text{\1}}|\mathbf{s}_{\text{\1}}, \mathbf{d}\_t) \quad \text{subject to} \quad 
\end{equation}
\begin{cases}
\begin{equation}
...
\begin{equation}
\sum_{i=1}^n w\_i = 1 \& \text{(budget constraint)}
\end{equation}
\end{cases}
\end{equation}
...
\begin{equation}
\pi_{\text{\1}}(\mathbf{d}_t|\mathbf{s}_{\text{\1}}) \quad \text{where} \quad \mathbf{d}_t \in \mathcal{D}_{\text{\1}}
\end{equation}
...
\begin{equation}
q(\theta) = \prod_{\text{\1}} \text{Bernoulli}(\theta_i; 1-p) \cdot \mathcal{N}(\theta_i; \mu_i, \sigma_i^2)
\end{equation}
...
\begin{equation}
V^{\pi}_{\text{\1}}(s, g) = \mathbb{E}^{\pi}\left[\sum_{t=0}^{\tau_g} \gamma^t r_{EOA,t} \Big| s_0 = s, \text{goal} = g\right]
\end{equation}
\end{equation}
...
\end{equation}
\begin{equation}
\item Learning rate schedule: $\sum_{\text{\1}} \alpha\_t = \infty, \sum_{\text{\1}} \alpha\_t^2 < \infty$
\end{equation}
...
\begin{equation}
For two-timescale stochastic approximation with step sizes $\alpha_t^{(\theta)}, \alpha\_t^{(w)}$:
\end{equation}
\begin{equation}
...
\begin{equation}
\lim_{t \to \infty} (\theta_t, w_t) = (\theta^\textit{, w^}) \quad \text{almost surely}
\end{equation}
\end{equation}
...
\begin{equation}
\item Sector concentration: $\sum_{j \in \text{sector}} |w\_j| \leq 0.3$
\end{equation}
...
\begin{equation}
\mathcal{L}_{\text{penalty}} = \mathcal{L}_{\text{base}} + \sum_{\text{\1}} \lambda_k \max(0, g_k(w))^2
\end{equation}
\end{equation}
...
\begin{equation}
\text{Risk Budget}_t = \text{VaR}_{\max} \times \exp\left(-\frac{\text{Drawdown}_t}{\text{\1}\2}\right)
\end{equation}
...
|---|---|---|
| Implementation Shortfall | $\text{sgn}(Q) \cdot \left(\frac{\text{\1}\2} - P\_{\text{\1}}(1+r\_f \Delta t)}{P\_{\text{\1}}}\right)$ | Per trade/window [18] |
