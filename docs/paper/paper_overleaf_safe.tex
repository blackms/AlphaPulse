...
\1ath
\pi_{EOA}(a\_{EOA}|\mathbf{s}_{EOA}, \mathbf{d}\_t) \quad \text{subject to} \quad 
\begin{cases}
...
subject to:
\begin{itemize}
\item $\pi_{SPA}$ is $\{\mathcal{F}_t\}$-adapted
...
where $\mathcal{H}_h$ is the LSTM hypothesis n units.
...
\item Learning rate schedule: $\sum_{t} \alpha\_t = \infty, \sum_{t} \alpha\_t^2 < \infty$
