...
class MockExchange(IExchange):
    ...
    def fetch_ohlcv(
        ...
        for i in range(limit or 100):
            ...
            price = base_price * (1 + np.random.normal(0, 0.001))  # 0.1% volatility
            candle = [
                timestamp,
                price * (1 - 0.001),  # Open
                price * (1 + 0.001),  # High
                price * (1 - 0.001),  # Low
                price,                # Close
                np.random.normal(100, 10)  # Volume
            ]
...
class PaperTradingSystem:
    ...
    async def _process_symbol(self, symbol: str, df: pd.DataFrame):
        ...
        # Trading logic
        if signal > 0.7 and (current_position is None or current_position.quantity <= 0):
            # Strong buy signal
            self._place_trade(symbol, OrderSide.BUY, current_price)
        elif signal < 0.3 and current_position is not None and current_position.quantity > 0:
            # Strong sell signal
    ...
    def _place_trade(self, symbol: str, side: OrderSide, current_price: float):
        ...
        position_value = account_value * 0.01
